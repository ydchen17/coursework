...
The manual screening will check the details of the intervention, especially whether it involves support or training for carers. Among all the 79 studies for manual screening, we exclude 46 studies based on the following reasons:

```{r}
selection_of_studies <- unique(read.xlsx('Dataset_intervention_screening_final.xlsx', 'screening'))
knitr::kable(table(selection_of_studies$Exclusion.reason))
```
...
```{r}
selected_studies <- unique(read.xlsx('Dataset_intervention_screening_final.xlsx', 'result'))
...
```{r}
selected_studies_list <- unique(selected_studies$Study.Identifier)
...
# subset by selected studies
df_selected <- 
  dataset[dataset$'Study.Identifier' %in% as.list(selected_studies_list), ]
# df_selected[reserved_cols]
```
